...
from transformers import MT5ForConditionalGeneration, MT5Tokenizer, Trainer, AdamW, TrainingArguments, EarlyStoppingCallback
...
#%% we use 42 seeds for reproducibility
random.seed(42)
...
#%% read data
training_data = pd.read_csv('train3.csv', encoding='latin-1')
ground_truth = pd.read_csv('now5.csv', encoding='latin-1') 
test_data = pd.read_csv('a-test.csv', encoding='latin-1') 




 #%% Remove any unnecesary characters/things
...
training_data['word'] = training_data['word'].apply(preprocess_text)
training_data['stem'] = training_data['stem'].apply(preprocess_text)
ground_truth['word'] = ground_truth['word'].apply(preprocess_text)
ground_truth['label'] = ground_truth['label'].apply(preprocess_text)
test_data['word'] = test_data['word'].apply(preprocess_text)


##%% Exception list
exception = ['oromo', 'axmaaro', 'tigree', 'lelkase', 'biciidyahan', 'tigre', 'uraago', 'jigjiga', 'arbe',
                           'maraykan', 'ciidan', 'wadan', 'ustareeliya', 'ustareliya', 'xildhibaan', 'garowe',
                           'wajaale', 'wajale', 'Caraaye', 'Xassan', 'soomaaliya', 'Kheyre', 'harboole', 'somaliland', 'bakayle',
                           'geelle', 'geele', 'helsinki', 'koow', 'gaarane', 'hawiya',
                           'ababa', 'mareexaan', 'bare', 'barre', 'cismaan', 'cusmaan', 'kenya', 'garoowe', 'abgaal',
                           'laasqoray', 'laasqorey', 'calmadow', 'Keeniya', 'kismaayo', 'kismayo', 'cadaado', 'cadado',
                           'gaalkacayo', 'galkacayo', 'maqale', 'shire', 'khaatumo', 'ssc-khaatumo', 'xassan', 'xassan',
                           'eriteriya', 'canada', 'cigaal', 'mareykan', 'maraykanka', 'sabriina', 'sabriin',
                           'saabriina', 'rayaale', 'hogaamiye', 'taliye', 'madaxweyne', 'onlf', 'muqdisho', 'shabeelay', 'ogaadeen',
                           'ogaden', 'ogadeen', 'galadiid', 'beercaano', 'dhooboweyn', 'lasdharkaynle',
                           'laasdharkaynle', 'dhanaan','caano', 'baydhaba', 'baydhabo', 'buula', 'qaylo ', 'afrikaan', 'gaadsan', 'ciidogale',
                           'shiine','ciise', 'gare', 'garre', 'murursade', 'murursadde', 'sade', 'saleebaan', 'xawaadle',
                           'hawiye','daarood', 'leelkase', 'dhulbahante', 'ciidagale', 'habarjeclo', 'muuse', 'awrtable',
                           'ortable', 'majeerteen', 'majerteen', 'kaskiiqabe', 'bartire', 'yabaree', 'yabarree', 'wayteen',
                           'weyteen', 'jaarso', 'gugundhabe', 'raxanweyn', 'raxanwayn', 'cabdale', 'baare', 'shube', 'isaayaas',
                           'aadaan', 'kow', 'laba', 'labo', 'sadex', 'afar', 'shan', 'lix', 'todobo', 'sideed', 'sagaal', 'toban', 'labaatan',
                           'sodan', 'afartan', 'kontan', 'lixdan', 'todobaatan', 'sideetan',
                           'sagaashan', 'boqol', 'kun', 'milyan', 'sodonaad', 'bilyan', 'kumaad', 'boqolaad', 'afartanaad',
                           'shanaad', 'lixaad', 'sideedaad', 'sideetanaad', 'sagaashanaad','dhar','shaqo',
                           'todobaad', 'tobnaad', 'milyanaad', 'sagaalaad','madaxwayne','africa','africa','pakistan',
                           ]
                          #madaxwayn, africa, soomaaliya
#%% We access the mT5-small model
class HybridStemmer:
    def __init__(self):
    ...
    def somstem(word):
        ...
        if word in exception:
            ...
            return word
        elif word.endswith('dulqaadanayay'):
            ...
            return word
        elif word.endswith('macalumaad'):
            return word
        elif word.endswith('dhamaad'):
            return word
            ...
            return word
        suffixes = [
            'yaashaan', 'yaashii', 'jireen', 'jirteen', 'jirnay', 'jiray', 'jirin', 'jirney', 'jirnay', 'jirteen',
            'sanahay', 'sanahey', 'soco',
            'hooda', 'heena', 'hayga', 'hiina', 'asheena', 'shooda', 'shiina', 'shaada', 'gooda', 'gaaga', 'gayga',
            'aashaa', 'aasha', 'aashay', 'ashay',
            'hooda', 'heena', 'hayga', 'hiina', 'asheena', 'shooda', 'shiina', 'shaada', 'gooda', 'gaaga', 'gayga',
            'yeeli', 'aashood', 'aashayda', 'ashayda', 'aashiin', 'ashiin',
            'iin', 'karnaa', 'kartaan', 'maayaan', 'maysaan', 'heena', 'kareen', 'hiisa', 'kartaa', 'maysid', 'jirtay',
            'yiin', 'sadeen', 'koodii', 'sanayn', 'mayaa', 'yahan', 'meey', 'sheen', 'shiin', 'shay',
            'nahay', 'yahay', 'yahow', 'yoow', 'mayso', 'mayno', 'karo', 'kara', 'karaa', 'maayo', 'badan', 'tahay',
            'tiiye', 'taasi', 'nimadu', 'sanaan', 'nimada', 'nimadda',
            'taan', 'ow', 'tooyo', 'sanaa', 'keenu', 'keen', 'heed', 'hood', 'heen', 'keed', 'kaas', 'kaan', 'kiina',
            'saan', 'yaan', 'ayaan', 'yan', 'yeen', 'yen', 'itaan', 'taann', 'taanno', 'nimo',
            'bay', 'ay', 'ahay', 'daro', 'na', 'yay', 'tay', 'tey', 'nay', 'ey', 'san', 'yaal', 'nina', 'nin', 'no',
            'seen', 'leey', 'doo',
            'kan', 'kaa', 'kii', 'ki', 'kuu', 'ku', 'kee', 'dee', 'ke', 'gee', 'ga', 'ka', 'aal', 'aay', 'she', 'oow',
            'oob', 'baa', 'ba', 'sii', 'tan',
            'tid', 'sid', 'id', 'niin', 'da', 'gu', 'gii', 'no', 'tu', 'tii', 'to', 'du', 'so', 'iyiin', 'ayay',
            'dii', 'taa', 'sa', 'naa', 'eey', 'eed', 'kood', 'jir', 'le', 'leh', 'la', 'to', 'do', 'aad', 'sha',
            'dha', 'das', 'yaa',
            'taa', 'tee', 'koow', 'yey', 'say', 'teen', 'een', 'seen', 'ya', 'yihiin', 'sa', 'ah', 'oo', 'ayaa', 'na',
            'dunna',
            'aa', 'ka', 'ee', 'tee', 'yaabo', 'eenu', 'iinu', 'iisu', 'yaro', 'aysay',
            'kaasi', 'haasi', 'hiisa', 'uu', 'gooda', 'kasta', 'keede', 'dhaw', 'dooda ', 'kiisi', 'ta', 'keedii',
            'saa', 'kaana','tihiin','gaalkacayo',
        ]
        # Our main Body.
        while True:
            #%% Rule 2
            ...
            for suffix in suffixes:
                if word.endswith(suffix):
                    ...
                    word = word.replace('xirfado', 'xirfad')
                    return word
                elif word == 'bilaawday':
                    ...
                    word = word.replace('gabayadii', 'gabay')
                    return word
                elif word == 'qaraxu':
                    ...
                    return word
                elif word == 'fannaaniin':
                    word = word.replace('fannaaniin', 'fannaan')
                    return word
                elif word == 'siyaasadiisa':
                    word = word.replace('siyaasadiisa', 'siyaasad')
                    return word
                elif word == 'faqrada':
                    word = word.replace('faqrada', 'faqrad')
                    return word
                    ...
                    return word
                elif word == 'astaamihiisu':
                    word = word.replace('astaamihiisu', 'astaan')
                    return word
                elif word == 'kawayn':
                    ...
                    return word
                elif word == 'faqradihii':
                    word = word.replace('faqradihii', 'faqrad')
                    return word
                elif word == 'bilow':
                    return word
                elif word == 'dibjir':
                    return word
                elif word == 'macluumaad':
                    return word
                elif word == 'liitay':
                    ...
                    return word
                elif word.endswith('yada'):
                    word = word.replace('yada', '')
                    return word
                    ...
                    return word
                elif word == 'Isticmaalidda':
                    word = word.replace('Isticmaalidda', 'Isticmaal')
                    return word
                elif word == 'mareegaha':
                ...
                # pulular
                    ...
                    word = word.replace('fadhiyada', 'fadhi')
                    return word
                elif word == 'faderaal':
                    return word
                elif word == 'fadaraal':
                    return word
                elif word == 'dheelitirnaan':
                    word = word.replace('dheelitirnaan', 'dheelitir')
                    return word
                elif word == 'dheelitirnan':
                    word = word.replace('dheelitirnan', 'dheelitir')
                    return word
                elif word == 'dheellitirnan':
                    word = word.replace('dheellitirnan', 'dheelitir')
                    return word
                elif word == 'dhaqangalin':
                    word = word.replace('dhaqangalin', 'dhaqangal')
                    return word
                elif word == 'dhammaad':
                    return word
                elif word == 'afrikaan':
                    ...
                    word = word.replace('hadla', 'hadal')
                    return word
                elif word.endswith('ahmiyadda'):
                    word = word.replace('ahmiyadda', 'ahmiyad')
                    return word
                elif word.endswith('aqalo'):
                    word = word.replace('aqalo', 'aqal')
                    return word

                elif word.endswith('aftiyaha'):
                    word = word.replace('aftiyaha', 'afti')
                    return word
                    ...
                    return word
                elif word.endswith('albaabadu'):
                    word = word.replace('albaabadu', 'albaab')
                    return word

                elif word.endswith('dooda'):
                    word = word.replace('dooda', 'dood')
                    return word
                elif word.endswith('fadhig'):
                    word = word.replace('fadhig', 'fadhi')
                    return word
                    ...
                    word = word.replace('cayaaraan', 'cayaar')
                    return word
                elif word == 'qabsa':
                    ...
                    word = word.replace('socdaan', 'socod')
                    return word
                    ...
                    word = word.replace('qaalmaha', 'qaali')
                    return word
                elif word == 'guryahooda':
                    word = word.replace('guryahooda', 'guri')
                    return word
                elif word == 'laaba':
            ...
            if matched_suffix:
                ...
                break
        consonants = 'btjxkhsdcgfqlmnwyr'  # Somali Alphabets
        # %% Rule 3 starts here
        if word.endswith('mayn') and len(word) > 3:
        ...
        elif word.endswith('xirfa') and len(word) > 3:
            word = word.replace('xirfad', 'xirfa')  # newly added
        ...
        #elif word.endswith('adani') and len(word) > 3:
            if len(word) > 6 and word[-6] != 'q' and word[-6] != 'c':
                 word = word.replace('adani', 'o')
        elif word.endswith('adani') and len(word) > 3 and word[-6] == 'q' and word[-6] == 'c' and word[-7] == 'i':
        #elif word.endswith('adani') and len(word) > 3 and len(word) > 7 and word[-7] == 'i' and (word[-6] == 'q' or word[-6] == 'c'):
        ...
        elif word.endswith('aha') and len(word) > 3 and word != 'heesaha' and word[-4] != 'd' and word[-4] != 'l' and \
                word[-4] != 'b':
            ...
            return word
        elif word.endswith('ii') and len(word) > 3:
        ...
        elif word.endswith('naan') and len(word) > 4 and word[-5] != 'o':
        ...
        elif word.endswith('daan') and len(word) > 3:
            word = word.replace('aan', '')
        elif word.endswith('qaan') and len(word) > 3:
            word = word.replace('aan', '')
        elif word.endswith('gaan') and len(word) > 3:
            word = word.replace('aan', '')
        elif word.endswith('ran') and len(word) > 2:
            word = word.replace('an', '')
        elif word.endswith('tadan') and len(word) > 3:
            word = word.replace('tadan', 'to')
        elif word.endswith('abtig') and len(word) > 3:
            word = word.replace('abtig', 'abti')
        elif word.endswith('dhaqam') and len(word) > 3:
            word = word.replace('dhaqam', 'dhaqan')
        elif word.endswith('cabit') and len(word) > 3:
            word = word.replace('cabit', 'cabid')
        elif word.endswith('iga') and len(word) > 3:
            word = word.replace('iga', 'i')
        elif word.endswith('arrima') and len(word) > 3:
            word = word.replace('arrima', 'arrin')
        elif word.endswith('arima') and len(word) > 3:
            word = word.replace('arima', 'arrin')
        elif word.endswith('in') and word[-3] != 'l' and word[-3] != 'i' and word[-3] != 'd' and len(word) > 4:
        #elif word.endswith('in') and len(word) > 4 and word[-3] not in ('l', 'i', 'd'):
            return word[:-1]
        elif word.endswith('ig') and len(word) > 4:
            word = word.replace('g', ' ')
        elif word.endswith('nahu') and len(word) > 4:
            word = word.replace('nahu', 'ne ')
        elif word.endswith('a') and len(word) > 4:
            return word[:-1]
        elif word.endswith('aqalad'):
            return word[:-2]
        elif word.endswith('nimo') and len(word) > 4:
            return word[:-4]
        elif word.endswith('keedii') and len(word) > 3:
            return word[:-6]
        elif word.endswith('ka') and len(word) > 3:
            return word[:-2]
        elif word.endswith('lii') and len(word) > 3:
            return word[:-2]
        elif word.endswith('iy') and len(word) > 3:
            return word[:-1]
        elif word.endswith('yeelo') and len(word) > 3:
            return word[:-5]
        elif word.endswith('dhint') and len(word) > 3:
            word = word.replace('dhint', 'dhimasho')
        #elif word.endswith('an') and word[-3] != 'd' and len(word[-3]) > 3: #old
        #elif word.endswith('an') and word[-3] != 'd' and len(word) >= 4: #changed from 3 to 4
            #word = word.replace('an', '')
        # take care about this rule, it caused some problems.
        elif word.endswith('an') and len(word) > 3 and len(word[-3]) > 3 and word[-3] != 'd':    
            word = word.replace('an', '')
        elif word == 'ark':
        ...
        # word = word.replace('a', '')
        #%% Rule 4
        if len(word) > 3 and all(char in consonants for char in word[-3:]) and word[-3] != 'y':
            word = word[:-3] + word[-3] + 'a' + word[-2:]
            return word
        elif word.endswith('e') and len(word) > 3 and word[-2] != 'b' and word[0] != 'h':
            word = word.replace('e', '')
        # elif word.endswith('a'):
        # word = word.replace('a', '')
        if len(word) > 2 and word[-1] in consonants and word[-2] in consonants and word[-1] != 'h' and word[
            -2] != 'y' and word[-2] != 'w':
            word = word[:-1]

         #%% Prefix definition
        #%% Rule 5
        prefix = ['ja', 'fud', 'waa', 'yar', 'cus', 'dhaa', 'ad', 'gu', ]
        for pre in prefix:
    ...
    #%% Rule 6 will start here, the mT5-small model
    def train(self, training_data):
        training_data['input_text'] = 'stem: ' + training_data['word']
        training_data['target_text'] = training_data['stem']

        train_texts, val_texts, train_labels, val_labels = train_test_split(
            training_data['input_text'].tolist(),
            training_data['target_text'].tolist(),
            test_size=0.2,
            random_state=42
        )
        ...
        train_dataset = Dataset.from_dict({
            ...
            'labels': self.tokenizer(train_labels, truncation=True, padding=True, max_length=128)['input_ids']
        })

        val_dataset = Dataset.from_dict({
            'input_ids': val_encodings['input_ids'],
            'attention_mask': val_encodings['attention_mask'],
            'labels': self.tokenizer(val_labels, truncation=True, padding=True, max_length=128)['input_ids']
        })
    ...
    #%% We save model for future use
    def save_model(self):
        logger.info("Model Saved Successfully")
        self.model.save_pretrained('./5_mt5_hybrid_fine_tuned')
        self.tokenizer.save_pretrained('./5_mt5_hybrid_fine_tuned')

     #%% For future use we load saved model
    def load_model(self):
        logger.info("Model loaded Successfully")
        self.model = MT5ForConditionalGeneration.from_pretrained('./5_mt5_hybrid_fine_tuned').to(self.device)
        self.tokenizer = MT5Tokenizer.from_pretrained('./5_mt5_hybrid_fine_tuned')

    #%% Stemming function
    def stem(self, word):
        if len(word) <=3 or word in exception:
            return word

            # First, we use rule-based stemming
        rule_based_result = self.somstem(word)
        #We check if the word is in the training data stems
        if word in self.training_data['word'].values:
           return self.training_data.loc[self.training_data['word'] == word, 'stem'].values[0]
        if not rule_based_result or rule_based_result == word:
            input_text = 'stem: ' + word
            ...
            try:
                ...
                #We clean the output here.
                cleaned_somword = re.sub(r'<extra_id_\d>', '', decoded_output).strip()
                return cleaned_somword if cleaned_somword else word
            except Exception as e:
                logger.info(f"Error generating stem: {e}")
                return word
        return rule_based_result    

hybrid_stemmer = HybridStemmer()
try:
    hybrid_stemmer.load_model()
    print("Loaded existing model.")
except Exception as e:
    print("No model found. Training new model.")
    hybrid_stemmer.train(training_data)

#%% Evaluations
ground_truth['predicted_stem'] = ground_truth['word'].apply(hybrid_stemmer.stem)
accuracy = accuracy_score(ground_truth['label'], ground_truth['predicted_stem'])
precision = precision_score(ground_truth['label'], ground_truth['predicted_stem'], average='weighted', zero_division=0)
recall = recall_score(ground_truth['label'], ground_truth['predicted_stem'], average='weighted', zero_division=0)
f1 = f1_score(ground_truth['label'], ground_truth['predicted_stem'], average='weighted', zero_division=0)
...
#%%
test_data['predicted_stem'] = test_data['word'].apply(hybrid_stemmer.stem)  
formatted_output = test_data[['word', 'predicted_stem']]
formatted_output.to_csv('5_mT5_hybrid.csv', index=False)
print(formatted_output.head())
